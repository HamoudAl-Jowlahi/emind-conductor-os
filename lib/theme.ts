 ...
 * app/globals.css.
 */
export const THEMES = ['emind', 'mono', 'mono-light', 'dark', 'light', 'midnight', 'ember'] as const;
export type Theme = (typeof THEMES)[number];

/** What every fresh load gets until the user picks something else. */
export const DEFAULT_THEME: Theme = 'emind';
...
export const THEME_META: Record<Theme, { name: string; blurb: string; swatch: [string, string, string] }> = {
  ...
  emind: { name: 'eMind', blurb: 'monolith black with the eMind violet', swatch: ['#0a0a0a', '#7c5cff', '#2fd36f'] },
