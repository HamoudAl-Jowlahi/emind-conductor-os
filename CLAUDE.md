...
(formerly Founder OS — rebranded 2026-08-16; repo slug `emind-conductor-os`.)
...
Next.js 16 App Router (server components, Turbopack) + TypeScript + Tailwind +
better-sqlite3 (`data/founder-os.db`, WAL, auto-seeded on first touch) +
Zod + Vitest 4.

**Next 16 migration (2026-08-16).** Upgraded from 14.2 → 16.3. Two rules the
framework now enforces, both of which fail *silently* if broken:

- `params` and `searchParams` are **Promises**. Always
  `const { id } = await params;` — reading `params.id` directly yields
  `undefined` with no throw and no warning, which degrades every dynamic route
  into "unknown …". Pinned by `tests/async-params.test.ts`; pass
  `Promise.resolve({…})` from tests, the way the framework actually calls them.
- Native/CJS packages go in top-level **`serverExternalPackages`**
  (`next.config.mjs`), not `experimental.serverComponentsExternalPackages`.
  `better-sqlite3` breaks if it is ever bundled.

better-sqlite3 needs a prebuilt binary matching the Node ABI: **`^13` is
required on Node ≥ 24** (`^11` only builds from source, which needs MSVC).
Never run `npm audit fix --force` here — that is what jumped the majors.
...
- THEME: **eMind (`emind`) is the default** (2026-08-16, `DEFAULT_THEME` in
  `lib/theme.ts`; bare `:root` in `app/globals.css` carries the emind tokens).
  It is Monolith's token set with one change: `--accent` is the brand violet
  `#7c5cff` (4.55:1 on the canvas — AA). The discipline still holds — violet
  marks **identity** (active nav, primary fills, the mark, `--brain-1`), the
  status trio `ok`/`warn`/`err` still owns every expression of **state**, and
  the funnel keeps its grey ramp. Brand mark: `components/EmindMark.tsx`, the
  single-stroke "em" monogram inheriting `currentColor` (no baked-in hex, so
  every colorway re-inks it). Monolith Signal (`mono`) stays pickable. "Terminal" (`dark`) — the phosphor-green command deck on
